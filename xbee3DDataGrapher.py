...
def main():
	...
	ax3=plt.subplot(333)
	ax4=plt.subplot(337)
	ax5=plt.subplot(323,projection='3d')
	ax6=plt.subplot(324,projection='3d')
	ax7=plt.subplot(338)
	ax8=plt.subplot(339)
	plt.ion()
	plt.show()
		...
		while True:
			...
			try:	
				if (len(points[0]) == len(points[7])) and (len(points[0]) == len(points[8])) and (len(points[0]) == len(points[9])):
					ax5.cla()
					if count<100:
						ax5.plot(points[7],points[8],points[9],'k')
					else:
						ax5.plot(points[7][-25:-1],points[8][-25:-1],points[9][-25:-1],'k')
					textstr = '\n'.join((("Latest X: {}".format(points[7][-1])),("Latest Y: {}".format(points[8][-1])),("Latest Z: {}".format(points[9][-1]))))
					ax5.text(0.05, 0.95,0.05, textstr, transform=ax5.transAxes, fontsize=14, verticalalignment='top', bbox=props)
					ax5.set_xlim(-2,2)
					ax5.set_ylim(-2,2)
					ax5.set_zlim(-2,2)
					ax5.xaxis._axinfo['jugled'] = (0,0,0)
					ax5.spines['right'].set_position('zero')
			...
			try:	
				if (len(points[0]) == len(points[10])) and (len(points[0]) == len(points[11])) and (len(points[0]) == len(points[12])):
					ax6.cla()
					if count<100:
						ax5.plot(points[10],points[11],points[12],'k')
					else:
						ax6.plot(points[10][-25:-1],points[11][-25:-1],points[12][-25:-1],'k')
					textstr = '\n'.join((("Latest X: {}".format(points[10][-1])),("Latest Y: {}".format(points[11][-1])),("Latest Z: {}".format(points[12][-1]))))
					ax6.text(0.05, 0.95,0.05, textstr, transform=ax6.transAxes, fontsize=14, verticalalignment='top', bbox=props)
					ax6.set_xlim(-2,2)
					ax6.set_ylim(-2,2)
					ax6.set_zlim(-2,2)
					ax6.xaxis._axinfo['jugled'] = (0,0,0)
					ax6.spines['right'].set_position('zero')
